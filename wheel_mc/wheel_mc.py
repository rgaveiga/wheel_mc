...
def run_simulation(inputs: InputData | dict) -> SimulationData:
    ...
    r = (
        inputs.risk_free_rate / 12 * inputs.number_of_periods
    )  # Risk-free interest rate for T
    ...
    if (
        inputs.covered_calls_deadline < 1
        or inputs.covered_calls_deadline > days_per_period
    ):
        inputs.covered_calls_deadline = days_per_period

    # Generate price paths
    stock_prices = (r - 0.5 * vol**2.0) * t + vol * sqrt(t) * normal(
        0, 1, (inputs.number_of_trading_paths, T)
    )
            ...
            if len(purchaseprice) > 0:
                ...
                for l in range(day_1, m):
                    if 0.0 not in xc:
                        ...
                        if inputs.write_puts_if_no_calls and not writtencall:
                            writeput = True
